...
import { Sparkles, Send, Camera, Image as ImageIcon, Loader2, Bot, HelpCircle } from 'lucide-react';
            ...
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`rounded-2xl p-4 ${
                  msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-slate-50 text-slate-700 rounded-tl-none'
                }`}>
                  {msg.image && (
        ...
        <div className="p-6 border-t border-slate-50 bg-slate-50/50 space-y-4">
          {image && (
            <div className="relative inline-block">
              <img src={image} className="h-20 w-20 object-cover rounded-xl shadow-md" alt="Preview" />
              <button 
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 rounded-xl hover:shadow-md transition-all"
            >
              <Camera size={22} />
            </button>
            <input 
            ...
            <div className="flex-1 relative">
              <input 
                type="text" 
                placeholder="Pregunta algo técnico (ej. Protocolo falla de carga iPhone 13)..."
