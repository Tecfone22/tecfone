...
const AIAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: "Error al conectar con la IA de soporte." }]);
    } finally {
      setLoading(false);
      setImage(null);
    }
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-slate-400 p-4">
              ...
              <span className="text-xs font-bold">Consultando base de datos técnica...</span>
            </div>
          )}
        </div>
